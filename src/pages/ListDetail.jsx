...
import { fetchListById, fetchListItems, removeListItem, deleteList } from '../lib/api/lists'
...
export default function ListDetail() {
  ...
  const { data, loading, error, refetch } = useAsync(async () => {
    ...
    return { data: { list, items: items || [] }, error: null }
  }, [listId])
        ...
        {items.map((item, i) => {
          ...
          return (
            <div key={item.id} style={{ background: '#181818', border: '0.5px solid #222', borderRadius: 12, padding: '12px 14px', display: 'flex', alignItems: 'center', gap: 12 }}>
              ...
              <div onClick={() => variant && navigate(`/product/${variant.id}`)} style={{ flex: 1, minWidth: 0, cursor: variant ? 'pointer' : 'default' }}>
