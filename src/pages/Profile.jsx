    ...
    <div style={{ display: 'flex', flexDirection: 'column', height: '100%' }}>
      ...
      <div style={{ flex: 1, overflowY: 'auto' }}>
        ...
        <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', gap: 6, padding: '20px 16px 16px' }}>
          ...
          <div style={{ fontSize: 12, color: '#4a4a4a', ...sans }}>{currentUser.goal} · {currentUser.location}</div>
        </div>
        ...
        <div style={{ display: 'flex', borderBottom: '0.5px solid #1e1e1e', margin: '16px 0 0' }}>
          {[['tried','Tried'],['lists','Lists'],['reviews','Reviews']].map(([key, label]) => (
            <button key={key} onClick={() => setTab(key)} style={{ flex: 1, background: 'none', border: 'none', borderBottom: tab === key ? '1.5px solid #e8e4dc' : 'none', cursor: 'pointer', padding: '11px 0', fontSize: 12, ...sans, color: tab === key ? '#e8e4dc' : '#444', fontWeight: tab === key ? 500 : 400 }}>
              {label}
            </button>
          ))}
        </div>
