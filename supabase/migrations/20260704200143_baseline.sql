-- Stackd MVP schema
-- FROZEN as of Phase 9 -- supabase/migrations/ is now the source of truth.
-- This file is captured verbatim as migrations/20260704200143_baseline.sql
-- (marked applied against the dev project via `supabase migration repair`,
-- not actually re-run -- the tables already existed). Any new schema change
-- from here on is a new file via `supabase migration new <name>`, applied
-- with `supabase db push`, never a hand-run snippet or an edit to this file.
-- Kept as a single human-readable reference for what the schema looked like
-- at this point; assumes pgcrypto (gen_random_uuid) is available by default
-- on Supabase.
--
-- Written to always drop and rebuild everything it owns rather than assume
-- a clean database -- accurate for a truly fresh database only. Real user
-- data exists in the dev project (accounts, reviews, etc as of Phase 3.5);
-- this file must never be re-run wholesale against it.

drop view if exists variant_rating_summary cascade;
drop view if exists public_profiles cascade;
-- follows/review_likes were never created by this file -- leftovers from
-- the original pre-Phase-0 prototype schema (a follow graph and review
-- likes, neither ever built; the plan explicitly drops the follow graph).
-- Found empty and unreferenced by any app code during Phase 9's migration
-- baseline audit; dropped here so a fresh run cleans them up too.
